...
int main() {
	...
	int label;
	vector<int>ymin, ymax, xmin, xmax;
	EightConnections(waterShed, label, ymin, ymax, xmin, xmax);
	...
	for (int i = 0; i < label; i++)
	{
		if (/*(simi[i]<0.02) &&*/ (fuseratio[i] < 0.65))
			rectangle(origin, Point(xmin[i], ymin[i]), Point(xmax[i], ymax[i]), Scalar(0,0,255), 2, 8, 0);
	}
